...
"""
    ...
    md(
        """\
## 5. Full training (~90 min on Colab T4, measured)

This runs the canonical 10000-step training with FP16 AMP + cosine LR schedule. Final val PPL should land around 4.7–4.9.
...
> ⏱️ **Measured timing**: ~91 min on Colab free T4 (only ~10% slower than 83 min on a local RTX 3060 at batch=64). The gradient accumulation loop adds minimal overhead — original concern that it would double wall-clock turned out to be unfounded.
>
> 💡 If you don't want to wait the full ~90 min, **skip this cell** — the smoke test alone produced a `ckpt_step100.pt` you can use for the inference cells below (output quality will be much weaker, but the pipeline works end-to-end).
>
